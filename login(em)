...
//<![CDATA[
	...
	// [D] wah_start: 전체 교체하시고, 구코드 삭제해주세요
	var elNomemContents = $('.nomemContent'),
		elNomenTab = $('.pop_tab'),
		elNomenTabImgs = $('.pop_tab img');
	...
	elNomenTab.delegate('li > a', 'click', function(e) {
		...
		if($(elNomemContents.get(idx)).is(':hidden')){
			fnChangeImg(elNomenTabImgs[idx], true);
			fnChangeImg(elNomenTabImgs[(idx === 0) ? 1 : 0], false);
			elNomemContents.toggle();
			(idx === 0)? window.resizeTo('597','580'): window.resizeTo('597','950');
		}
...
<form name="reloadForm" method="get" action="">	<!-- [D] action="" 추가 -->
